...
public extension TimeSeries where DataSeriesType.PointType == DataPointType, DataSeriesType.PointType : Sampleable {
    ...
    init(from:Date, for duration:TimeInterval, every interval:TimeInterval, using dataSeries:DataSeriesType){
        self.dataSeries = dataSeries
        timeSeriesStart = from.timeIntervalSinceReferenceDate
        self.duration = duration
        self.interval = interval
        self.summarizer = MeasureValue<SourceType>()
...
public struct TimeSeries<DataSeriesType:DataSeries, DataPointType> {
    public typealias SourceType = DataSeriesType.PointType
    ...
    public var summarizer   : any Summarizer<SourceType, DataPointType> {
        didSet {
            update()
        }
    }
    ...
    public init(from:Date, for duration:TimeInterval, every interval:TimeInterval, using dataSeries:DataSeriesType, summarizer: any Summarizer<SourceType, DataPointType>){
        self.dataSeries = dataSeries
        timeSeriesStart = from.timeIntervalSinceReferenceDate
        self.duration = duration
        self.interval = interval
    ...
    mutating public func capture(_ value:SourceType, at time: TimeInterval = Date.now.timeIntervalSinceReferenceDate) throws(CaptureError) {
